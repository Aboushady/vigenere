...
object Main {
  ...
  val frequencies = Array(0.0812, 0.0123, 0.0103, 0.0355, 0.0828, 0.0147, 0.029, 0.0167, 0.0445, 0.0064, 0.0284, 0.0406, 0.029, 0.0692, 0.0326, 0.0118, 0.0, 0.0663, 0.0463, 0.0691, 0.0142, 0.0191, 0.0001, 0.0009, 0.004, 0.0001, 0.0001, 0.0001, 0.0001, 0.1507, 0.005, 0.0088)
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("----- Teachers ----")
    var teachersKey = calculateKeyFromManyCiphers(techersCiphers,190 to 200);
    println(teachersKey)
    techersCiphers.foreach(cipher => {
      println("----")
      println(decrypt(cipher,teachersKey))
    })

    println("----- Teachers Manualy modified ----")
    var fixedTeachersKey = "ooohh, vad en liten gumman kan gno. jämt och nu, är alltid trötta. vi står i, fast vi är nötta. man får stå på, om man ska få det som man helst vill ha det. ooohh, vad en liten gumma kan gno."
    println(fixedTeachersKey)
    techersCiphers.foreach(cipher => {
      println("----")
      println(decrypt(cipher,fixedTeachersKey))
    })
